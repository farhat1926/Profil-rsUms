...
// Function untuk mengelompokkan dokter berdasarkan spesialis
    ...
    fetch(`${API_URL}/jadwal`)
      ...
      .then((data) => {
        ...
        data.forEach((item) => {
          ...
          // Perbaikan: Konversi hari ke lowercase agar cocok dengan key di state jadwal
          if (item.hari && item.jam) {
            const hariKecil = item.hari.toLowerCase();
            if (result[item.id].jadwal[hariKecil]) {
              result[item.id].jadwal[hariKecil].push(item.jam);
            }
          }
        });
